...
namespace
{
    ...
    {
        using MouseEventType = InputGenerator::MouseEventType;
        switch (_eventType)
        {
            case MouseEventType::Drag:
                return buttonNormal(_button, _eventType) + uint8_t{0x20};
...
bool InputGenerator::mouseTransportX10(uint8_t _button, uint8_t _modifier, int _row, int _column)
{
    ...
    if (_row <= MaxCoordValue && _column <= MaxCoordValue)
    {
        ...
        uint8_t const row = SkipCount + static_cast<uint8_t>(_row);
        uint8_t const column = SkipCount + static_cast<uint8_t>(_column);
        append("\033[M");
        append(button);
        append(column);
        append(row);
