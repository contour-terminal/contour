Note: Setup Terminal with test-defaults
Note: UTF-8 is not enabled
Send: <27> [ 0 c 
Wait: 1
Read: <27> [ ? 6 5 ; 1 ; 4 ; 7 ; 9 ; 1 8 ; 2 1 ; 2 2 ; 2 9 ; 5 2 ; 3 1 4 c 
Done: 1
Send: <27> P $ q " p <27> \ 
Wait: 2
Read: <27> P 1 $ r 6 5 ; 1 " p <27> \ 
Done: 2
Send: <27> [ > c 
Wait: 3
Read: <27> [ > 6 5 ; 7 0 0 ; 0 c 
Done: 3
Send: <27> [ 1 ; 7 ; 0 , | 
Note: Max Operating Level: 5
Note: Cur Operating Level: 5
Note: Derived terminal-id: 525
Note: set_level(5)
Note: ...set_level(5) in=7, out=7, fsm=7
Send: <27> [ ? 1 l 
Send: <27> [ ? 3 l 
Send: <27> [ ? 4 l 
Send: <27> [ ? 5 l 
Send: <27> [ ? 6 l 
Send: <27> [ ? 7 h 
Send: <27> [ ? 8 l 
Send: <27> [ ? 4 0 h 
Send: <27> [ ? 4 5 l 
Send: <27> [ r 
Send: <27> [ 0 m 
Text: VT100 test program, version 2.7
Text:  (20251205)
Text: Line speed 38400bd 
Text: Choose test type:
Text: 
Text:           0. Exit
Text:           1. Test of cursor movements
Text:           2. Test of screen features
Text:           3. Test of character sets
Text:           4. Test of double-sized characters
Text:           5. Test of keyboard
Text:           6. Test of terminal reports
Text:           7. Test of VT52 mode
Text:           8. Test of VT102 features (Insert/Delete Char/Line)
Text:           9. Test of known bugs
Text:           10. Test of reset and self-test
Text:           11. Test non-VT100 (e.g., VT220, XTERM) terminals
Text:           12. Modify test-parameters
Text: 
Text:           Enter choice number (0 - 12): 
Read: 1 0 
Note: choice 10: Test of reset and self-test
Text: Menu 10: 
Text: Test of reset and self-test
Text: Choose test type:
Text: 
Text:           0. Exit
Text:           1. Reset to Initial State (RIS)
Text:           2. Invoke Terminal Test (DECTST)
Text:           3. Soft Terminal Reset (DECSTR)
Text: 
Text:           Enter choice number (0 - 3): 
Read: * 
Note: Selecting all choices
Note: choice 10.1: Reset to Initial State (RIS)
Text: Reset to Initial State (RIS)
Text: (VT100 & up, not recommended)
Text: 
Text: The terminal will now be RESET. 
Data: P u s h <32> < R E T U R N > 
Read: 
Send: <27> c 
Data: P u s h <32> < R E T U R N > 
Read: 
Note: choice 10.2: Invoke Terminal Test (DECTST)
Text: Invoke Terminal Test (DECTST)
Text: 
Text: The terminal is now RESET.  Next, the built-in confidence test
Text: will be invoked. 
Data: P u s h <32> < R E T U R N > 
Read: 
Send: <27> [ 2 ; 1 y 
Text: If the built-in confidence test found any errors, a code
Text: is visible above. 
Data: P u s h <32> < R E T U R N > 
Read: 
Note: choice 10.3: Soft Terminal Reset (DECSTR)
Text: Soft Terminal Reset (DECSTR)
Text: (VT220 & up)
Text: 
Text: The terminal will now soft-reset
Data: P u s h <32> < R E T U R N > 
Read: 
Send: <27> [ ! p 
Data: P u s h <32> < R E T U R N > 
Read: 
Text: Menu 10: 
Text: Test of reset and self-test
Text: Choose test type:
Text: 
Text:           0. Exit
Text:           1. Reset to Initial State (RIS)
Text:           2. Invoke Terminal Test (DECTST)
Text:           3. Soft Terminal Reset (DECSTR)
Text: 
Text:           Enter choice number (0 - 3): 
Read: 0 
Text: VT100 test program, version 2.7
Text:  (20251205)
Text: Line speed 38400bd 
Text: Choose test type:
Text: 
Text:           0. Exit
Text:           1. Test of cursor movements
Text:           2. Test of screen features
Text:           3. Test of character sets
Text:           4. Test of double-sized characters
Text:           5. Test of keyboard
Text:           6. Test of terminal reports
Text:           7. Test of VT52 mode
Text:           8. Test of VT102 features (Insert/Delete Char/Line)
Text:           9. Test of known bugs
Text:           10. Test of reset and self-test
Text:           11. Test non-VT100 (e.g., VT220, XTERM) terminals
Text:           12. Modify test-parameters
Text: 
Text:           Enter choice number (0 - 12): 
Read: 0 
Note: Cleanup & exit
Note: set_level(5)
Note: ...set_level(5) in=7, out=7, fsm=7
Send: <27> [ ? 1 l 
Send: <27> [ ? 3 l 
Send: <27> [ ? 5 l 
Send: <27> [ ? 6 l 
Send: <27> [ ? 7 h 
Send: <27> [ ? 8 h 
Send: <27> [ r 
Send: <27> [ 0 m 
Text: That's all, folks!
Text: 


