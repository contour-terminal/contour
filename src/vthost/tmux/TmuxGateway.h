...
class TmuxGateway final
{
  public:
    ...
    /// Whether @p text can travel as a quoted literal `send-keys -l` argument.
    ///
    /// False for any control byte: the control channel is line-oriented and the quoted-argument
    /// dialect escapes only `"` and `\`, so an embedded LF ends the command line inside the
    /// argument — the remainder of the payload then parses as fresh COMMANDS. Public and pure so
    /// the rule the emitter branches on is testable on its own.
    /// @param text The payload to check.
    /// @return true when every byte is a printable (non-control) character.
    [[nodiscard]] static bool isLiteralSafe(std::string_view text) noexcept;

    /// Sends literal text to @p pane in bounded `send-keys -l` batches
    /// (iTerm2's 1000-character cap per command). A batch that is not
    /// @ref isLiteralSafe goes out through @ref sendRawInput instead.
