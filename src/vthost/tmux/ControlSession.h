...
class ControlSession final: public vtworkspace::ModelEvents, public SessionStreamEvents
{
  public:
    ...
    /// @param id This connection's diagnostic identity, prefixed onto its every log line.
    ...
    /// Re-announces the owning window's layout, because @p session's grid has already moved.
    ///
    /// A resize is the one state change that reaches every client while only one asked for it, and
    /// the paths that carry it — a native client's `applyClientSize`, a detach's re-projection —
    /// raise no `vtworkspace::ModelEvents` at all. Without this a control-mode client kept
    /// rendering the geometry it last heard about while the shells behind the panes had already
    /// been resized. @see SessionStreamEvents::sessionResized.
