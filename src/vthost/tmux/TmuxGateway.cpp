...
bool TmuxGateway::isLiteralSafe(std::string_view text) noexcept
{
    // The control channel is strictly LINE-oriented and the quoted-argument dialect escapes only
    // `"` and `\` (@see splitCommandLine, and real tmux's own argument parser). A control byte
    // therefore cannot be carried literally at all: an embedded LF ends the command line
    // mid-argument, leaving an unterminated quote and turning the remainder of the payload —
    // the normal case for a multi-line paste — into commands the server then executes.
...
void TmuxGateway::sendKeys(uint64_t pane, std::string_view text)
{
    ...
    // literally (-l) with the quoting closed around embedded quotes.
    ...
    while (!text.empty())
    {
        ...
        // A batch carrying anything the quoting cannot express goes out in the hex form instead,
        // where every byte survives verbatim with no quoting rules in the way. Decided per batch
        // so an ordinary paste still travels as readable literal text.
        if (!isLiteralSafe(batch))
        {
            sendRawInput(pane, batch);
            continue;
        }
