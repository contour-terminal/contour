...
Task<void> scenario(LoopbackHarness* h)
{
    ...
    // 5c. A MULTI-LINE payload through sendKeys — the normal case for a paste. The control channel
    //     is line-oriented and the quoted-argument dialect escapes only `"` and `\`, so embedding
    //     the newline literally split the command mid-argument: `send-keys … -l -- "one` was
    //     rejected for its unterminated quote and `two"` was then parsed as a fresh COMMAND. The
    //     bytes must arrive whole, and nothing must reach the command dispatcher.
    mock.stdinBuffer().clear();
    h->gateway->sendKeys(paneId, "one\ntwo");
    co_await waitUntil(&h->loop, [&] { return mock.stdinBuffer().size() >= 7; });
    CHECK(mock.stdinBuffer() == "one\ntwo");

    // 5d. And a payload that tries to close the quote and append a command of its own stays DATA.
    mock.stdinBuffer().clear();
    h->gateway->sendKeys(paneId, "\"\nkill-server\n");
    co_await waitUntil(&h->loop, [&] { return mock.stdinBuffer().size() >= 14; });
    CHECK(mock.stdinBuffer() == "\"\nkill-server\n");
...
// The rule the emitter branches on, on its own: what may travel as a quoted literal `send-keys -l`
// argument and what must take the hex path instead. Everything below 0x20 (and DEL) is a control
// byte the line-oriented, quote-only-escaping dialect cannot express.
TEST_CASE("isLiteralSafe rejects exactly the bytes the quoting cannot express", "[vthost][gateway]")
{
    CHECK(TmuxGateway::isLiteralSafe(""));
    CHECK(TmuxGateway::isLiteralSafe("echo 'hi'"));
    // Quote and backslash ARE expressible — they are the two the escaper handles.
    CHECK(TmuxGateway::isLiteralSafe(R"(a"b\c)"));
    CHECK(TmuxGateway::isLiteralSafe("\x7e")); // '~', the last printable

    CHECK_FALSE(TmuxGateway::isLiteralSafe("one\ntwo")); // LF ends the command line
    CHECK_FALSE(TmuxGateway::isLiteralSafe("one\rtwo"));
    CHECK_FALSE(TmuxGateway::isLiteralSafe("\x1b[A"));
    CHECK_FALSE(TmuxGateway::isLiteralSafe(std::string { '\0' }));
    CHECK_FALSE(TmuxGateway::isLiteralSafe("\x7f"));
    // High bytes are UTF-8 continuation bytes, not control bytes: they stay literal.
    CHECK(TmuxGateway::isLiteralSafe("\xc3\xa4"));
}
