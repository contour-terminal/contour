...
/// A thin, observable QML wrapper around one node of a tab's vtmux::Pane tree.
///
...
/// proxies. The proxy holds no authority — it forwards reads to the underlying vtmux::Pane and
...
class PaneProxy: public QObject
{
    ...
    Q_PROPERTY(int orientation READ orientation NOTIFY changed) // vtmux::SplitState as int
  ...
  public:
    PaneProxy(TerminalSessionManager& manager, vtmux::PaneId id) noexcept: _manager { manager }, _id { id } {}
    ...
    [[nodiscard]] vtmux::PaneId modelId() const noexcept { return _id; }
    ...
    /// Caches the backing vtmux::Pane this proxy resolves to, so the property getters need not walk
    /// the tab tree on every QML binding evaluation. The manager re-points this from
    /// rebuildActiveTabPaneProxies() (which already walks the live tree) on each structural change,
    /// and clears it (nullptr) when the pane is pruned. @p pane may be null.
    void setPane(vtmux::Pane* pane) noexcept { _pane = pane; }
  ...
  private:
    ...
    [[nodiscard]] vtmux::Pane* pane() const noexcept { return _pane; }

    TerminalSessionManager& _manager;
    vtmux::PaneId _id;
    vtmux::Pane* _pane = nullptr;
