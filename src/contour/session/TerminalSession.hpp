...
class TerminalSession: public QAbstractItemModel, public vtbackend::Terminal::Events
{
  ...
  private:
    ...
    /// Puts the config file under the watcher, and reports it if that fails.
    ///
    /// One spelling for both arms -- the constructor's first one and onConfigReload()'s re-arm after a
    /// save dropped the path -- because they are the same operation with the same consequence: a
    /// failure here is PERMANENT and silent, since nothing else ever arms the watch. Guarded on
    /// isWatchingConfigFile(), so the re-arm is a no-op after a plain in-place write, which never
    /// loses the path (addPath() warns on a duplicate, and a warning is a build break here).
    ///
    /// From onConfigReload() this must run AFTER the reload, never beside it -- @see onConfigReload,
    /// which sequences the two and says why writing the call after its post() silently inverts them.
    void armConfigFileWatch();
