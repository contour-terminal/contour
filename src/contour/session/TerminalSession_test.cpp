          ...
          "[contour][session][clipboard][permission]")
{
    ...
    {
        // CRLF must become LF exactly as the immediate path does it: under bracketed paste a stray CR
        // reads as Enter, so a large CRLF script would EXECUTE line by line instead of being inserted.
        auto const line = QStringLiteral("echo hello\r\n");
        clipboard->setText(line.repeated(static_cast<int>(((qsizetype { 1024 } * 512) / line.size()) + 1)));
        session->pasteFromClipboard(1, /*strip=*/false);
        session->applyPendingPaste(/*allow=*/true, /*remember=*/false);

        auto const& written = mockPtyOf(*session).stdinBuffer();
        CHECK(written.contains("echo hello\n"));
        CHECK_FALSE(written.contains("\r"));
    }
