...
namespace
{
    ...
    /// so bytesPerLine() is not width*4 in general, while the wire format is tightly packed.
    [[nodiscard]] std::string tightlyPackedRgba(QImage const& image)
    {
        ...
        {
            auto const* const row = reinterpret_cast<char const*>(image.constScanLine(y));
            out.append(row, rowSize);
        }
                               ...
                               Format format)
{
    ...
    switch (format)
    {
        ...
        case Format::Rgba:
            return Capture { .content = tightlyPackedRgba(image), .pixelSize = extentOf(image) };
