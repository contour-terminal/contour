...
// ModelEvents -> begin/end* mapping the controller uses, wrapped in a QAbstractItemModelTester. The
...
class TabListModel: public QAbstractListModel, public ModelEvents
{
  public:
    ...
    // {{{ QAbstractListModel — mirror of WindowController
    ...
    {
        // Mirrors WindowController::notifyRatioChanged: look the split node's proxy up by PaneId
          ...
          "[contour][gui][model][tablabel]")
{
    ...
    {
        QSignalSpy const spy(&m, &QAbstractItemModel::dataChanged);
        REQUIRE(spy.isValid());
