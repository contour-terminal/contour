...
void TerminalSession::pasteFromClipboard(unsigned count, bool strip)
{
    if (QClipboard* clipboard = QGuiApplication::clipboard(); clipboard != nullptr)
    {
        ...
        if (text.size() > 10000000)
        {
            ...
            sessionLog()("Clipboard contains huve text. Requesting permission.");
            return;
        }

        sessionLog()("Size of text: {}", text.size());
        auto stippedText = strip_if(text.toStdString(), strip);
        if (stippedText.empty())
            sessionLog()("Clipboard does not contain text.");
        else if (count == 1)
            terminal().sendPaste(string_view { stippedText });
        else
        {
            string fullPaste;
            for (unsigned i = 0; i < count; ++i)
                fullPaste += stippedText;
