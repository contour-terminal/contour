...
namespace
{
                                     ...
                                     vector<uint8_t>& outputBitmap) noexcept
    {
        ...
        for (auto i: ::ranges::views::iota(size_t { 0 }, unbox(outputSize.height)))
        {
            for (auto j: ::ranges::views::iota(size_t { 0 }, unbox(outputSize.width)))
            {
                //  calculate area average
                std::array<unsigned int, NumComponents> components { {} };
                unsigned int count = 0;
                for (auto y: ::ranges::views::iota(i * factor,
                                                   min((i + 1) * factor, unbox<size_t>(inputSize.height))))
                {
                    uint8_t const* p =
                        inputBitmap.data() + (y * unbox<size_t>(inputSize.width) * 4) + (j * factor * 4);
                    for ([[maybe_unused]] auto _: ::ranges::views::iota(
                             j * factor, min((j + 1) * factor, unbox<size_t>(inputSize.width))))
                    {
                        ++count;
                        for (auto componentIndex: ::ranges::views::iota(size_t { 0 }, NumComponents))
