...
/// A synthetic BDF font, built in memory and served through FontMemoryRef.
///
...
///
/// The instance owns the font bytes; FontMemoryRef only borrows them, so it must outlive the shaper.
class BDFFont
{
  public:
    ...
    BDFFont(std::string identifier, bool monospace, std::vector<BDFGlyph> const& glyphs):
    ...
    {
        return FontMemoryRef { .identifier = _identifier, .data = gsl::span<uint8_t> { _bytes } };
    }

    /// The one size this font may be loaded at. BDF is a fixed-size bitmap format, so requesting any
    /// other size fails outright.
    static constexpr auto Size = FontSize { 9.0 };
    static constexpr auto Dpi = DPI { .x = 96, .y = 96 };

  private:
    [[nodiscard]] static std::vector<uint8_t> render(bool monospace, std::vector<BDFGlyph> const& glyphs)
    {
