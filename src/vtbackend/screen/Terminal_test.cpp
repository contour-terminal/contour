...
TEST_CASE("Terminal.AutoScrollOnUpdate", "[terminal]")
{
    ...
    // The setting is spelled "on update", and typing is not an update -- it is the user acting. A
    // prompt scrolled off screen that stays off screen while the user types into it is the failure
    // this pins: Enter, space and ordinary characters all reach the application, so all three must
    // bring the page back regardless of what the setting says about output.
    SECTION("Enter snaps to the bottom even with autoScrollOnUpdate=No")
    {
        terminal.setAutoScrollOnUpdate(vtbackend::AutoScrollOnUpdate::No);
        terminal.viewport().scrollUp(LineCount(2));
        REQUIRE(terminal.viewport().scrolled());
    ...
    SECTION("space snaps to the bottom even with autoScrollOnUpdate=No")
    {
        terminal.setAutoScrollOnUpdate(vtbackend::AutoScrollOnUpdate::No);
        terminal.viewport().scrollUp(LineCount(2));
        REQUIRE(terminal.viewport().scrolled());

        terminal.sendCharEvent(U' ',
    ...
    SECTION("a typed character snaps to the bottom even with autoScrollOnUpdate=No")
    {
        // The same rule as the Enter case above, exercised through the char path.
        terminal.setAutoScrollOnUpdate(vtbackend::AutoScrollOnUpdate::No);
        terminal.viewport().scrollUp(LineCount(2));
        REQUIRE(terminal.viewport().scrolled());
...
TEST_CASE("Terminal.HardReset.DropsTheLiveViewportOffset", "[terminal]")
{
    // RIS empties every grid, so an offset the user had scrolled to then addresses scrollback that no
    // longer exists -- the out-of-range state Grid::render asserts against and Grid::rowAt would
    // otherwise resolve by wrapping its ring buffer onto unrelated storage. The SAVED per-page offsets
    // were already cleared here; the live one was not, and nothing else drops it: setScreen(Primary)
    // early-returns when the cursor page is already 0, so bufferChanged() -- the one path that consults
    // autoScrollOnUpdate -- never runs on the common RIS.
    //
    // Deliberately asserted with autoScrollOnUpdate No, since that is the setting under which the
    // stale offset used to survive: declining a snap means keeping a position, and after RIS there is
    // no position left to keep.
    auto mc = MockTerm { PageSize { LineCount(4), ColumnCount(6) }, LineCount(10) };
    auto& terminal = mc.terminal;
    ...
    CHECK(!terminal.viewport().scrolled());
    CHECK(terminal.viewport().scrollOffset() == vtbackend::ScrollOffset(0));
    CHECK(terminal.primaryScreen().historyLineCount() == LineCount(0));
}

TEST_CASE("Terminal.AltScreen.EnteredEvenWhileDecpccmIsReset", "[terminal]")
{
    // DECPCCM decouples the DRAWN page from the cursor page among the pages of ONE buffer. Entering the
    // alternate screen is a buffer switch, not a page flip, so the drawn page has to follow it whatever
    // the mode says. Gated on DECPCCM alone, `CSI ?64l` followed by `CSI ?1049h` left _displayedPage on
    // the primary page while _currentScreenType, currentScreen() and therefore
    // Viewport::scrollingDisabled()/historyLineCount() had all moved to the alternate one -- the user
    // looking at the primary page, unable to scroll it, measured against a grid it does not belong to.
    auto mc = MockTerm { PageSize { LineCount(4), ColumnCount(6) }, LineCount(10) };
    auto& terminal = mc.terminal;

    mc.writeToScreen("\033[?64l"); // DECPCCM reset
    REQUIRE(!terminal.isModeEnabled(vtbackend::DECMode::PageCursorCoupling));

    mc.writeToScreen("\033[?1049h");
    REQUIRE(terminal.isAlternateScreen());
    CHECK(&terminal.displayedPage() == &terminal.alternateScreen());

    // And back out again: leaving is the same buffer switch in the other direction.
    mc.writeToScreen("\033[?1049l");
    REQUIRE(!terminal.isAlternateScreen());
    CHECK(&terminal.displayedPage() == &terminal.primaryScreen());
}

TEST_CASE("Terminal.TopAnchoredRegion.PartialScrollKeepsViCursorOnScreen", "[terminal]")
{
    // The sibling case below parks the viewport, so the viewport follows the scroll and
    // onViewportChanged() clamps the Vi cursor on the way. This is the branch where it does NOT: the
    // viewport sits at the bottom, so mustFollow is false, and a PARTIAL top-anchored region makes
    // wholePageMoved false too -- the viewport deliberately stays put and nothing runs the clamp.
    //
    // The cursor was still shifted once per scroll, with no bound of its own, so it walked off the top
    // of the page into scrollback the viewport is not showing: invisible, and unbounded until
    // clampToHistory()'s floor. Keeping it on its text is right only while that text is on screen;
    // once the text has gone the cursor has to stop at the edge, which is the rule
    // onViewportChanged() applies everywhere else.
    auto mc = MockTerm { PageSize { LineCount(10), ColumnCount(8) }, LineCount(50) };
    auto& terminal = mc.terminal;

    for (auto const i: std::views::iota(1, 15))
        mc.writeToScreen(std::format("h{:02}\r\n", i));

    terminal.setAutoScrollOnUpdate(vtbackend::AutoScrollOnUpdate::No);

    // Park the SCREEN cursor mid-page and enter Normal mode there, rather than walking the Vi cursor
    // up with `k`: every `k` runs Viewport::makeVisibleWithinSafeArea(), which scrolls -- and a
    // scrolled viewport is the parked branch the sibling case already covers, not this one.
    mc.writeToScreen("\033[6;1H");
    terminal.inputHandler().setMode(vtbackend::ViMode::Normal);

    // Placed directly rather than with `k`: walking up past the top row scrolls the viewport, which
    // would PARK it and take the other branch -- the one the sibling case already covers. Mid-page for
    // the same reason: Viewport::makeVisibleWithinSafeArea() scrolls for a cursor near either edge.
    INFO("cursor=" << terminal.normalModeCursorPosition().line.value
                   << " offset=" << terminal.viewport().scrollOffset().value);
    REQUIRE(!terminal.viewport().scrolled());
    REQUIRE(terminal.normalModeCursorPosition().line == LineOffset(5));

    // A partial region anchored at the top: rows 1..8 of a 10-row page, so the rebase boundary lands
    // above the page bottom (wholePageMoved is false) and the cursor sits inside it.
    mc.writeToScreen("\033[1;8r");

    // More scrolls than the cursor has rows above it, so an unclamped shift goes negative.
    for ([[maybe_unused]] auto const _: std::views::iota(0, 8))
        mc.writeToScreen("\033[S");

    // The invariant, stated as the viewport itself defines it: the Normal-mode cursor sits on a row
    // the viewport draws. The viewport never moved here, so this is simply [0, pageSize).
    auto const scrollOffset = terminal.viewport().scrollOffset().as<LineOffset>();
    CHECK(terminal.normalModeCursorPosition().line >= -scrollOffset);
    CHECK(terminal.normalModeCursorPosition().line
          <= boxed_cast<LineOffset>(terminal.pageSize().lines) - LineOffset(1) - scrollOffset);
}
