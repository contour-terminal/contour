...
class Message
{
  public:
    ...
    [[nodiscard]] HeaderMap const& headers() const noexcept { return _headers; }

    /// Looks up a header by key, returning a pointer to its value or nullptr if absent.
    [[nodiscard]] std::string const* header(std::string const& key) const noexcept
    {
    ...
    [[nodiscard]] Data const& body() const noexcept { return _body; }

    /// Moves the body out of the message (invalidates the message body).
