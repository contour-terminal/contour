...
{
    // The pixel formats have numbers but no implementation; an application asking for one is told so
    // rather than met with silence.
    for (auto const format: { "2"sv, "3"sv, "4"sv, "5"sv, "255"sv })
    {
...
TEST_CASE("Screenshot.reply.is_a_PM_and_never_an_OSC", "[screenshot]")
{
    ...
    auto const messages = collect([&](auto const& sink) { writeReply(request, "hi\n", sink); });
...
TEST_CASE("Screenshot.reply.echoes_the_resolved_region_one_based", "[screenshot]")
{
    ...
    auto const messages = collect([&](auto const& sink) { writeReply(request, "x", sink); });

    REQUIRE(messages.size() == 2);
    // Pid=12, Ps=1 (Data), then the region back in the units the request used, then Pf=1.
    CHECK(messages[0].starts_with("\033^533;12;1;1;1;5;20;1;"));
}
...
TEST_CASE("Screenshot.reply.payload_is_base64_and_round_trips", "[screenshot]")
{
    ...
    auto const request =
        Request { .id = 0,
                  ...
                  .format = Format::PlainText };
    auto const messages = collect([&](auto const& sink) { writeReply(request, content, sink); });
...
TEST_CASE("Screenshot.reply.ends_with_an_end_of_data_message", "[screenshot]")
{
    ...
    {
        auto const messages = collect([&](auto const& sink) { writeReply(request, "a", sink); });
        REQUIRE(messages.size() == 2);
        CHECK(messages[1] == "\033^533;5;0\033\\");
    }
    SECTION("an empty screenshot is still terminated")
    {
        auto const messages = collect([&](auto const& sink) { writeReply(request, "", sink); });
...
TEST_CASE("Screenshot.reply.chunks_reassemble_either_way", "[screenshot]")
{
    ...
    auto const request = Request { .id = 0,
                                   .area = Rect { .top = Top(0),
                                                  .left = Left(0),
                                                  .bottom = Bottom(0),
                                                  .right = Right(0) },
