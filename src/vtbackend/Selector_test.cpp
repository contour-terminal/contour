...
TEST_CASE("Selector.Reactivate", "[selector]")
{
    auto term = MockTerm(PageSize { LineCount(3), ColumnCount(11) }, LineCount(5));
    auto& screen = term.terminal.primaryScreen();
    auto selectionHelper = TestSelectionHelper(screen);
    ...
    SECTION("reactivate extends completed linear selection forward")
    {
        auto const from = CellLocation { .line = LineOffset(0), .column = ColumnOffset(2) };
        auto const to = CellLocation { .line = LineOffset(0), .column = ColumnOffset(5) };
        auto selector = LinearSelection(selectionHelper, from, []() {});
        (void) selector.extend(to);
        selector.complete();
        CHECK(selector.state() == Selection::State::Complete);

        // Re-activate and extend forward to a new position.
        selector.reactivate();
        CHECK(selector.state() == Selection::State::InProgress);

        auto const extendTo = CellLocation { .line = LineOffset(1), .column = ColumnOffset(3) };
        (void) selector.extend(extendTo);
        selector.complete();

        auto selectedText = TextSelection { screen };
        renderSelection(selector, selectedText);
        CHECK(selectedText.text == "345,67890\nab,c");
    }

    SECTION("reactivate extends completed linear selection backward")
    {
        auto const from = CellLocation { .line = LineOffset(1), .column = ColumnOffset(5) };
        auto const to = CellLocation { .line = LineOffset(1), .column = ColumnOffset(8) };
        auto selector = LinearSelection(selectionHelper, from, []() {});
        (void) selector.extend(to);
        selector.complete();

        selector.reactivate();
        auto const extendTo = CellLocation { .line = LineOffset(0), .column = ColumnOffset(1) };
        (void) selector.extend(extendTo);
        selector.complete();

        // Selection _from stays at original from (1,5), _to moves to extendTo (0,1).
        // Since extendTo < from, ranges are computed with ordered points: (0,1) to (1,5).
        auto selectedText = TextSelection { screen };
        renderSelection(selector, selectedText);
        CHECK(selectedText.text == "2345,67890\nab,cde");
    }

    SECTION("reactivate does nothing when not complete")
    {
        auto const from = CellLocation { .line = LineOffset(0), .column = ColumnOffset(0) };
        auto selector = LinearSelection(selectionHelper, from, []() {});
        CHECK(selector.state() == Selection::State::Waiting);

        // Should not change state if not Complete.
        selector.reactivate();
        CHECK(selector.state() == Selection::State::Waiting);
    }
}
