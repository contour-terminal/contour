...
void ChannelPty::feed(std::string_view data)
{
    {
        auto const lock = std::lock_guard { _mutex };
        // Reclaim the already-read prefix before appending. Reclaiming ONLY on a full drain
        // bounded nothing: a pane streaming continuously is fed from one thread while the parser
        // pump reads on another, so at feed time the buffer is essentially never exactly drained
        // -- and every byte the pane had ever produced stayed resident. The threshold is what
        // AsyncBufferedReader and ImsgDecoder use for the same reason: it bounds worst-case
        // memory without paying an O(n) shift on every feed of the common small-chunk case.
        if (_outputReadOffset == _outputBuffer.size())
        {
            _outputReadOffset = 0;
            _outputBuffer.clear();
        }
        else if (_outputReadOffset >= CompactionThreshold)
        {
            _outputBuffer.erase(0, _outputReadOffset);
            _outputReadOffset = 0;
        }
...
std::size_t ChannelPty::bufferedOutputBytes() const
{
    auto const lock = std::lock_guard { _mutex };
...
int ChannelPty::resizesAfterClose() const noexcept
{
    auto const lock = std::lock_guard { _mutex };
