...
using muxserver::tmux::ControlOutput;
using muxserver::tmux::ControlOutputLimits;
using muxserver::tmux::escapeOutput;
...
TEST_CASE("escapeOutput escapes control bytes and backslash as three octal digits", "[muxserver][control]")
{
...
TEST_CASE("a notification queued before output flushes immediately", "[muxserver][control]")
{
    auto h = QueueHarness {};
...
TEST_CASE("a notification never lands inside or before an earlier output block", "[muxserver][control]")
{
    auto h = QueueHarness {};
...
TEST_CASE("the byte budget is shared fairly across panes with pending output", "[muxserver][control]")
{
    auto h = QueueHarness {};
...
TEST_CASE("a congested connection still makes writeMinimum progress", "[muxserver][control]")
{
    auto h = QueueHarness {};
    h.queue.enqueueOutput(1, std::string(40, 'a'), h.now);
...
TEST_CASE("pause-after drops the backlog and emits one %pause; %continue resumes", "[muxserver][control]")
{
    auto h = QueueHarness {};
...
TEST_CASE("a paused pane's dropped blocks do not gate later notifications", "[muxserver][control]")
{
    auto h = QueueHarness {};
    h.queue.setPauseAfter(100ms);
