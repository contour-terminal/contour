...
using namespace muxserver::imsg;
...
TEST_CASE("the header encodes 16 host-order bytes with len including itself", "[muxserver][imsg]")
{
...
TEST_CASE("a pending fd is claimed by the first marked header", "[muxserver][imsg]")
{
...
TEST_CASE("an unclaimed fd is closed when the next one arrives", "[muxserver][imsg]")
{
    auto closed = std::vector<int> {};
    {
        auto decoder = ImsgDecoder { RecordingCloser { &closed } };
...
TEST_CASE("a marked header without an fd is tolerated", "[muxserver][imsg]")
{
...
TEST_CASE("argv packs and unpacks per cmd_unpack_argv's rules", "[muxserver][imsg]")
{
...
TEST_CASE("the identify sequence a 3.7b client sends accumulates", "[muxserver][imsg]")
{
...
TEST_CASE("payload-shape violations and the acceptance matrix reject", "[muxserver][imsg]")
{
    auto state = IdentifyState {};
