...
TEST_CASE("MuxServer serves concurrent connections through the injected handler", "[muxserver][server]")
{
...
TEST_CASE("closing the server ends the accept loop", "[muxserver][server]")
{
    ...
    auto server = MuxServer { loop, std::move(*listener), muxserver::drainConnection };
