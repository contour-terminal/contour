...
struct PipeFds
{
    ...
    PipeFds(PipeFds const&) = delete;
    PipeFds& operator=(PipeFds const&) = delete;
    PipeFds(PipeFds&&) = delete;
    PipeFds& operator=(PipeFds&&) = delete;
...
TEST_CASE("PosixReadSelector.concurrent_cancel_during_wait_is_race_free")
{
    ...
    // race). Runs on every platform because PosixReadSelector is unconditionally defined.
