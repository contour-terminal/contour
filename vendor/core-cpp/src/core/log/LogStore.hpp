...
/// @file
/// Categorised logging: named categories, each enabled or not, writing through a sink.
///
/// A category is a process-wide object that registers itself by name, so a filter string
/// (configure()) can enable a set of them by name, as `LOG=net.*` does for core-cpp's test
/// binaries. Where the output goes and how a line looks is `<core/log/LogSink.hpp>`.
...
/// Builds one message of a category, and writes it to the category's sink when it is destroyed.
...
class Category
{
  public:
    ...
    [[nodiscard]] Sink& sink() const noexcept { return _sink.get(); }
  ...
  private:
    ...
    std::reference_wrapper<Sink> _sink;
};
...
class Sink
{
  public:
    ...
    /// The standard debug-logging sink: standard output, disabled until setEnabled(true).
    static Sink& console();
    /// A sink to standard error, enabled.
...
/// @return Every category that exists now, in the order they were constructed.
std::vector<std::reference_wrapper<Category>>& get();

/// @param categoryName The name to look up.
/// @return The category of that name, or nullptr if none exists.
Category* get(std::string_view categoryName);

/// Points every category that exists now at @p sink, which must outlive them or be replaced first.
void setSink(Sink& sink);

/// Gives every category that exists now the formatter @p f.
void setFormatter(Category::Formatter const& f);

/// Enables (or disables) the category named @p categoryName, if it exists.
void enable(std::string_view categoryName, bool enabled = true);

/// Disables the category named @p categoryName, if it exists.
void disable(std::string_view categoryName);

/// Applies a category filter; see its definition below.
...
inline void configure(std::string_view filterString)
{
    ...
    if (filterString == "all")
    {
        for (auto& category: get())
            category.get().enable();
    }
    else
    {
        auto const filters = core::split(filterString, ',');
        for (auto& category: get())
        {
...
inline MessageBuilder::MessageBuilder(Category const& cat, SourceLocation location):
...
{
    std::erase_if(get(),
                  [this](std::reference_wrapper<Category> const& each) { return &each.get() == this; });
}
...
/// The `error` category: enabled from the start, and kept enabled by configure().
auto inline errorLog = Category("error", "Error Logger", Category::State::Enabled);

#define errorLog() (::core::log::errorLog())
