...
async::Task<void> WriteQueue::flushThenClose()
{
    co_await pollUntil(&_loop, [state = _state] { return state->backlogBytes == 0 && !state->draining; });
    close();
}

async::Task<void> WriteQueue::waitUntilBacklogBelow(std::size_t watermark, std::function<bool()> callerDone)
{
...
async::Task<void> WriteQueue::drain(std::shared_ptr<State> state)
{
