...
/// timer, so it is deterministic under an injected @c platform::ManualClock.
...
namespace detail
{
    ...
    template <typename T>
    async::Task<void> captureResult(async::Task<T> work, std::shared_ptr<std::optional<T>> slot)
    {
    ...
    inline async::Task<void> timeoutArm(EventLoop* loop, std::chrono::milliseconds timeout)
    {
...
async::Task<std::optional<T>> withTimeout(EventLoop* loop,
                                          async::Task<T> work,
                                          std::chrono::milliseconds timeout)
{
    ...
    auto const winner = co_await async::whenAny(std::move(tasks));
    if (winner == 0)
...
inline async::Task<bool> withTimeout(EventLoop* loop,
