// SPDX-License-Identifier: Apache-2.0

/// @file multi-page-demo.cpp
/// Demonstrates a command palette / modal dialog overlay using DEC VT420 multi-page
/// support and DECCRA (Copy Rectangular Area).
///
/// The demo copies the current page's content to a scratch page (page 2) using DECCRA,
/// switches to that scratch page, draws a popup with an animated progress bar on top
/// of the copied content, and then switches back to page 1 — which was never modified.
/// This is the pattern a real command palette or overlay dialog would use.

#include <sys/ioctl.h>

#include <algorithm>
#include <chrono>
#include <cstdlib>
#include <iostream>
#include <string>
#include <string_view>
#include <thread>

#include <termios.h>
#include <unistd.h>

using namespace std::literals;
using namespace std::chrono_literals;

namespace
{

// -- Low-level terminal helpers -----------------------------------------------

void writeToTTY(std::string_view s) noexcept
{
    ::write(STDOUT_FILENO, s.data(), s.size());
}

/// Reads a terminal response up to (and including) the given @p sentinel character.
std::string readResponse(char sentinel, std::chrono::milliseconds timeout = 500ms)
{
    std::string response;
    auto const deadline = std::chrono::steady_clock::now() + timeout;

    while (std::chrono::steady_clock::now() < deadline)
    {
        char ch {};
        auto const n = ::read(STDIN_FILENO, &ch, 1);
        if (n == 1)
        {
            response += ch;
            if (ch == sentinel)
                break;
        }
    }
    return response;
}

/// Sends DECRQDE and checks for a valid response to detect multi-page support.
/// Returns true if the terminal supports multi-page, false otherwise.
bool detectMultiPageSupport()
{
    // DECRQDE: CSI " v — request display extent
    writeToTTY("\033[\"v");

    // Expected response: CSI Ph ; Pw ; 1 ; 1 ; Pp " w
    auto const response = readResponse('w');
    return !response.empty() && response.back() == 'w' && response.find("\"w") != std::string::npos;
}

// -- RAII terminal raw mode ---------------------------------------------------

struct RawMode
{
    termios saved {};

    RawMode()
    {
        ::tcgetattr(STDIN_FILENO, &saved);
        auto raw = saved;
        raw.c_lflag &= ~(ICANON | ECHO);
        raw.c_cc[VMIN] = 0;
        raw.c_cc[VTIME] = 1; // 100 ms read timeout for non-blocking reads.
        ::tcsetattr(STDIN_FILENO, TCSANOW, &raw);
    }

    ~RawMode() { ::tcsetattr(STDIN_FILENO, TCSANOW, &saved); }

    RawMode(RawMode const&) = delete;
    RawMode& operator=(RawMode const&) = delete;
};

// -- Drawing helpers ----------------------------------------------------------

/// Moves the cursor to (row, col) — 1-based.
std::string moveTo(int row, int col)
{
    return std::format("\033[{};{}H", row, col);
}

/// Sets foreground and background colors using SGR (256-color mode).
std::string setColor(int fg, int bg)
{
    return std::format("\033[38;5;{}m\033[48;5;{}m", fg, bg);
}

constexpr auto Reset = "\033[0m"sv;
constexpr auto Bold = "\033[1m"sv;
constexpr auto HideCursor = "\033[?25l"sv;
constexpr auto ShowCursor = "\033[?25h"sv;

// -- Page navigation sequences ------------------------------------------------

/// PPA — Page Position Absolute: switch to 1-based page number.
std::string pagePositionAbsolute(int pageNumber)
{
    return std::format("\033[{} P", pageNumber);
}

/// DECCRA — Copy Rectangular Area: copies the entire visible area to a target page.
/// @param rows Number of rows in the terminal.
/// @param cols Number of columns in the terminal.
/// @param sourcePage Source page (0 = current page, 1-based otherwise).
/// @param targetPage Target page (1-based).
std::string copyPageTo(int rows, int cols, int sourcePage, int targetPage)
{
    return std::format("\033[1;1;{};{};{};1;1;{}$v", rows, cols, sourcePage, targetPage);
}

// -- Popup / progress bar drawing ---------------------------------------------

constexpr int PopupWidth = 44;
constexpr int PopupHeight = 9;
constexpr int BarWidth = PopupWidth - 6; // usable cells for the bar

/// Repeats a UTF-8 string @p s exactly @p count times.
std::string repeat(std::string_view s, int count)
{
    std::string result;
    result.reserve(s.size() * static_cast<size_t>(count));
    for (auto i = 0; i < count; ++i)
        result += s;
    return result;
}

/// Draws the popup frame centered at the given origin.
void drawPopupFrame(int topRow, int leftCol)
{
    // Colors: white text on a dark blue background.
    auto const color = setColor(15, 17);
    auto const hLine = repeat("─", PopupWidth - 2);

    // Top border
    writeToTTY(std::format("{}{}{}┌{}┐", moveTo(topRow, leftCol), color, Bold, hLine));

    // Middle rows (empty body)
    for (auto row = 1; row < PopupHeight - 1; ++row)
    {
        writeToTTY(std::format("{}│{}│", moveTo(topRow + row, leftCol), std::string(PopupWidth - 2, ' ')));
    }

    // Bottom border
    writeToTTY(std::format("{}└{}┘", moveTo(topRow + PopupHeight - 1, leftCol), hLine));

    // Title
    auto constexpr Title = " Command Palette "sv;
    auto const titleCol = leftCol + (PopupWidth - static_cast<int>(Title.size())) / 2;
    writeToTTY(std::format("{}{}", moveTo(topRow, titleCol), Title));

    // Hint text
    auto constexpr Hint = "Loading items..."sv;
    auto const hintCol = leftCol + (PopupWidth - static_cast<int>(Hint.size())) / 2;
    writeToTTY(
        std::format("{}{}{}{}", moveTo(topRow + 2, hintCol), setColor(153, 17), std::string(Hint), Reset));

    writeToTTY(std::string(Reset));
}

/// Draws the progress bar at the given percentage (0–100).
void drawProgressBar(int topRow, int leftCol, int percent)
{
    auto const barRow = topRow + 4;
    auto const barCol = leftCol + 3;

    auto const filled = std::clamp(percent * BarWidth / 100, 0, BarWidth);
    auto const empty = BarWidth - filled;

    // Green filled portion, dark gray empty portion.
    writeToTTY(moveTo(barRow, barCol));
    writeToTTY(setColor(16, 40)); // black on green
    writeToTTY(std::string(static_cast<size_t>(filled), ' '));
    writeToTTY(setColor(250, 236)); // light gray on dark gray
    writeToTTY(std::string(static_cast<size_t>(empty), ' '));
    writeToTTY(std::string(Reset));

    // Percentage label
    auto const label = std::format("{:>3}%", percent);
    auto const labelCol = leftCol + (PopupWidth - static_cast<int>(label.size())) / 2;
    writeToTTY(std::format("{}{}{}{}{}", moveTo(topRow + 6, labelCol), setColor(15, 17), Bold, label, Reset));
}

} // namespace

int main()
{
    if (!::isatty(STDIN_FILENO))
    {
        std::cerr << "Error: stdin is not a terminal.\n";
        return EXIT_FAILURE;
    }

    auto const rawMode = RawMode {};
    writeToTTY(HideCursor);

    // -- Feature detection via DECRQDE ----------------------------------------
    if (!detectMultiPageSupport())
    {
        writeToTTY(ShowCursor);
        std::cerr << "Error: DEC multi-page support is not available in this terminal.\n";
        return EXIT_FAILURE;
    }

    // -- Query terminal size --------------------------------------------------
    struct winsize ws {};
    ::ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws);
    auto const rows = static_cast<int>(ws.ws_row);
    auto const cols = static_cast<int>(ws.ws_col);

    // -- Copy current page content to scratch page 2 via DECCRA ----------------
    // Source page 0 = current page. This preserves whatever is currently visible
    // (unknown/uncontrolled content) as an exact copy including all cell attributes.
    writeToTTY(copyPageTo(rows, cols, 0, 2));

    // -- Switch to page 2 — user sees the copied content ----------------------
    writeToTTY(pagePositionAbsolute(2));
    // Note: no clear — page 2 now holds a faithful copy of the original content.

    // -- Draw popup overlay on top of the copied content ----------------------
    auto const topRow = (rows - PopupHeight) / 2 + 1;
    auto const leftCol = (cols - PopupWidth) / 2 + 1;

    drawPopupFrame(topRow, leftCol);

    // -- Animate progress bar from 0% to 100% --------------------------------
    for (auto percent = 0; percent <= 100; ++percent)
    {
        drawProgressBar(topRow, leftCol, percent);
        std::this_thread::sleep_for(30ms);
    }

    std::this_thread::sleep_for(400ms);

    // -- Switch back to page 1 — original content was never touched -----------
    writeToTTY(pagePositionAbsolute(1));

    writeToTTY(ShowCursor);
    writeToTTY(moveTo(rows, 1));
    writeToTTY("\n");

    return EXIT_SUCCESS;
}
