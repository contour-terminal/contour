...
- [x] `vthost::SessionHost` (SessionId → {Pty, Terminal}, implements `vtworkspace::ModelEvents`,
...
- [x] 3b. leb128 PDU codec, version handshake, side tables (hyperlinks, image cells),
      `FetchImage`/`ImageData`/`ImageGone`, session-state snapshot (`src/vthost/proto/`)
...
- [x] rewritten-imsg codec (`vthost/imsg/`): pure framing (16-byte host-order
...
- [x] simplify: the three native-protocol decode loops share `vthost::PduPump`
