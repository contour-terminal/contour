...
- **The Arabic presentation-form fallback is absent.** Mapping a run onto U+FE70..U+FEFF when the
  resolved font has no `arab` GSUB would keep a legacy font usable. Every Arabic font that ships on
  the platforms tested does have `arab` GSUB, so the branch could not be exercised in the wild and
  was left out rather than written blind. The DirectWrite half of the shaping change is written but
  **unverified**: it cannot be compiled on the machine it was written on and must be checked on
