...
| | |
|---|---|
| Tool | `typos`, version pinned in `scripts/check-spelling.sh` (`TYPOS_VERSION`) |
| Config | `_typos.toml` at the repository root |
| Runner | `scripts/check-spelling.sh` |
| Locally | `ctest -L lint`, or run the script directly |
...
Perl word-splitter with its own dictionaries), while `scripts/check-spelling.sh` ran `cspell`
...
- **Every entry needs an inline justification comment.** `scripts/check-spelling.sh` enforces this
...
Bump `TYPOS_VERSION` in `scripts/check-spelling.sh`. There is no second place to change, and no CI
