...
- **The daemon already owns a complete `vtworkspace::SessionModel`** (windows→tabs→pane trees),
  *implements* `vtworkspace::ModelEvents`, and already has server-side `createTab()`/`splitActivePane()`
...
- **Only cursor-shape (DECSCUSR), OSC 7 cwd, and palette/OSC 4/10/11 lack a live `Events`
  signal.** They always accompany screen output, so they can be **pull+diffed** in
...
- [x] **B1. `LayoutState` PDU (F1, outbound).** New tag-11 PDU (forward-compatible, no codec bump):
      ...
      `vtworkspace::SessionModel` (`serializeLayout`/`serializePaneTree`), pushes it **leading the attach
...
- 2026-07-22 · Windows/clangcl-release · **B2 reconstruction reworked to reuse the shared realizer,
  ...
  `vtworkspace::realizeLayoutTab` (DRY: one tree-reconstruction path, shared with the daemon and the
